	...
	<table>
		<tr>
			...
			<th style="text-align: right;">
				...
				<div style="font-size: 14px">Reporte Placas</div>
			</th>
			<hr>
		</tr>
	</table>
	...
	<table>
	    ...
	    @foreach($data as $filter)
	  	<tr>
			...
			<td>{{ $filter['dni'] }}</td>
			<td>{{ $filter->address['car']['license_plate'] }}</td>
			<td>{{ $filter->address['car']['brand'] }}</td>
			<td>{{ $filter->address['car']['model'] }}</td>
	  	</tr>
		@endforeach
	</table>
</body>
