                    ...
                    <v-data-table
                        ...
                        <template v-slot:top>
                            <v-toolbar flat color="transparent" class="round">
                                <v-toolbar-title>Carnet</v-toolbar-title>
                                <v-divider
                                    class="mx-4"
                                    inset
                                    vertical></v-divider>
        ...
        methods: {
            async initialize() {
                try {
                    ...
                    const response = await this.$store.dispatch("driving_license/INDEX", request);
                } catch (error) {
                    console.log(error);
                }
            },
            ...
            eventdialog(message) {
                ...
                if (message === 'registro') {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Carnet editado con exito',
                    })
                    this.active_l = false;
                    this.edit_l = false;
                } else {
            ...
            editItem(item) {
                this.data_dependent = {
                    ...item,
                    position_array: this.ItemsDriving.indexOf(item)
                }
            ...
            async deleteItem(item) {
                try {
                    Swal.fire({
                        title: 'Esta seguro que desea borrar el carnet ?',
                        ...
                        console.log(request, '*******')
                        const response = await this.$store.dispatch("driving_license/DELETE", request);
                              Swal.fire({
                                title: 'Eliminado!',
                                text: "Su carnet fue elimando con exito",
                                icon: 'success',
                                confirmButtonColor: '#E53935',
                              })
                        }
