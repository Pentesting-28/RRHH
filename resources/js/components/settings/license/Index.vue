            ...
            <v-data-table
              ...
              <template v-slot:top>
                <v-toolbar flat color="transparent" class="round">
                    <v-toolbar-title>Tipo de carnet</v-toolbar-title>
                    <v-divider
                    class="mx-4"
                    inset
                    vertical></v-divider>
...
<script>
  ...
  import dialogLicense from './Dialog';
  ...
  export default {
    ...
    computed: {
      ...
      ItemsPosition() {
        return this.$store.getters["licenseType/GET_LICENSE"];
      },
    },
    ...
    methods: {
      async initialize () {
        try {
          const response = await this.$store.dispatch("licenseType/INDEX_LICENSE");
        } catch (error) {
          console.log(error);
        }
      },
      ...
      async deleteItem (item) {
        try {
          ...
          console.log(request,'*******')
          const response = await this.$store.dispatch("licenseType/DELETE", request);
        } catch (error) {
          console.log(error);
        }
