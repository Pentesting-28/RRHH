            ...
            <v-data-table
              ...
              <template v-slot:top>
                <v-toolbar flat color="transparent" class="round">
                    <v-toolbar-title>Acreedor</v-toolbar-title>
                    <v-divider
                    class="mx-4"
                    inset
                    vertical></v-divider>
...
<script>
  ...
  import dialogCreditor from './Dialog';
  ...
  export default {
    ...
    computed: {
      formTitle () {
        return this.edit === false ? 'Nuevo Acreedor' : 'Editar Acreedor'
      },
      ItemsCreditor() {
    ...
    methods: {
      ...
      eventdialog(message) {
        if (message === 'registro') {
          Swal.fire({
              icon: 'success',
              title: 'Acreedor creado con exito',
            })
          this.active = false;
          this.edit = false;
          ...
          Swal.fire({
              icon: 'success',
              title: 'Acreedor editado con exito',
            })
          this.active = false;
          this.edit = false;
        }else {
      ...
      editItem (item) {
        this.data_edit = {
          ...item,
          position_array: this.ItemsCreditor.indexOf(item)
        }
      ...
      async deleteItem (item) {
        try {
            Swal.fire({
                title: 'Esta seguro que desea borrar el acreedor ?',
                ...
                if (result.value) {
                    ...
                    const response = await this.$store.dispatch("creditor/DELETE", request);
                      Swal.fire({
                        title: 'Eliminado!',
                        text: "Su acreedor fue elimando con exito",
                        icon: 'success',
                        confirmButtonColor: '#E53935',
                      })
                }
