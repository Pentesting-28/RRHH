            ...
            <v-data-table
              ...
              <template v-slot:top>
                <v-toolbar flat color="transparent" class="round">
                    <v-toolbar-title>Metodos de Pago</v-toolbar-title>
                    <v-divider
                    class="mx-4"
                    inset
                    vertical></v-divider>
...
<script>
  ...
  export default {
    ...
    computed: {
      formTitle () {
        return this.edit === false ? 'Nuevo metodo de pago' : 'Editar Metodo de pago'
      },
      ItemsPosition() {
        return this.$store.getters["payment_method/GET_PAYMENT_METHOD"];
      },
    },
    ...
    methods: {
      async initialize () {
        try {
          const response = await this.$store.dispatch("payment_method/INDEX_PAYMENT_METHOD");
        } catch (error) {
          console.log(error);
        }
      },
      ...
      async deleteItem (item) {
        try {
          ...
          console.log(request,'*******')
          const response = await this.$store.dispatch("payment_method/DELETE", request);
        } catch (error) {
          console.log(error);
        }
